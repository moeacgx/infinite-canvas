...
const blockedHostSuffixes = [".localhost", ".local", ".internal", ".lan", ".home"];

export async function resolvePublicProxyTarget(rawTarget: string): Promise<PublicProxyTarget> {
    let url: URL;
    try {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error("WebDAV 仅支持 HTTP 或 HTTPS");
    if (url.username || url.password) throw new Error("WebDAV 地址不能包含用户名或密码");
    if (url.hash) throw new Error("WebDAV 地址不能包含片段标识");

    const hostname = stripIpv6Brackets(url.hostname).toLowerCase();
    if (!hostname || hostname === "localhost" || blockedHostSuffixes.some((suffix) => hostname.endsWith(suffix))) throw new Error("WebDAV 地址不能指向本机或内网主机");
    ...
    if (!resolved.length || resolved.some((item) => isBlockedIpAddress(item.address))) throw new Error("WebDAV 地址解析到了本机、内网或保留地址");

    const unique = Array.from(new Map(resolved.map((item) => [`${item.family}:${item.address}`, item])).values());
    return { url, addresses: unique };
}

export function createPinnedLookup(addresses: PublicProxyTarget["addresses"]): LookupFunction {
    return ((_hostname, options, callback) => {
        const requestedFamily = typeof options === "number" ? options : options?.family;
        const candidates = addresses.filter((item) => !requestedFamily || item.family === requestedFamily);
        if (!candidates.length) {
            callback(new Error("WebDAV 目标地址不可用"), "", 4);
            return;
        }
...
export function isBlockedIpAddress(rawAddress: string) {
    ...
    // 只允许全球单播 IPv6，并排除文档网段与 ORCHID 等保留范围。
...
function ipv6ToBigInt(address: string) {
    ...
    return tokens.reduce((value, token) => (value << BigInt(16)) | BigInt(token), BigInt(0));
}
