...
export function networkFailureMessage({ fallback, code, requestUrl, pageProtocol }: NetworkFailure) {
    ...
    if (isMixedContent(requestUrl, pageProtocol)) return `${fallback}：当前页面使用 HTTPS，浏览器禁止直连 HTTP 接口；请改用 HTTPS 地址`;
    return `${fallback}：浏览器未收到接口响应。通常是第三方接口未允许 CORS/OPTIONS、HTTPS 证书异常、DNS 或网络不可达；请让接口放行当前站点来源，或改用支持 CORS 的接口`;
}
