...
export function getProxyUrl(url: string): string {
    ...
    try {
        ...
        if (isLocalNetworkHost(parsed.hostname) || (typeof window !== "undefined" && parsed.host === window.location.host)) return url;
    } catch {
        return url;
    }
...
export async function downloadRemoteImage(url: string, signal?: AbortSignal) {
    ...
    if (!blob.type.startsWith("image/")) throw new Error("代理返回的内容不是图片");
    return blob;
}
...
export async function uploadRemoteImageToServer(url: string, filename: string): Promise<UploadedImage> {
    const blob = await downloadRemoteImage(url);
...
export async function imageToDataUrl(image: { url?: string; dataUrl?: string; storageKey?: string }, signal?: AbortSignal) {
    ...
    for (const url of urls) {
        ...
        try {
            const result = await blobToDataUrl(await downloadRemoteImage(url, signal));
            signal?.throwIfAborted();
            return result;
        } catch (error) {
...
export async function deleteStoredImages(keys: Iterable<string>) {
    ...
    const assetKeys = new Set(
        useAssetStore
            .getState()
            .assets.map((asset) => (asset.kind !== "text" ? asset.data.storageKey : null))
