...
function VideoSettingsPortal({ buttonRect, panelRef, placement, theme, config, onConfigChange, frameOptions, resourceOptions, metadata, firstFrameNodeId, lastFrameNodeId, onFrameChange, onMetadataChange, visualOnly }: { buttonRect: DOMRect; panelRef: RefObject<HTMLDivElement | null>; placement: CanvasVideoSettingsPopoverProps["placement"]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; config: AiConfig; onConfigChange: CanvasVideoSettingsPopoverProps["onConfigChange"]; frameOptions: CanvasVideoFrameOption[]; resourceOptions: CanvasVideoResourceOption[]; metadata?: CanvasNodeMetadata; firstFrameNodeId?: string; lastFrameNodeId?: string; onFrameChange?: CanvasVideoSettingsPopoverProps["onFrameChange"]; onMetadataChange?: CanvasVideoSettingsPopoverProps["onMetadataChange"]; visualOnly: boolean }) {
    ...
    const kling = resolveKlingCanvasVariant(config, model);
    const isKIEKlingV3 = kling?.provider === "kie" && kling.variant === "v3";
    const isKlingMotionControl = modelKey(model).includes("motion-control");
    const isKlingV3 = kling?.variant === "v3";
...
function resolveKlingCanvasVariant(config: AiConfig, modelName: string) {
    const key = modelKey(modelName);
    const channelText = activeVideoChannelText(config);
    if (key === "kling-v2-6") return { provider: channelText.includes("kie") ? "kie" : "apimart", variant: "v26" } as const;
    if (key === "kling-v3") return { provider: channelText.includes("kie") ? "kie" : "apimart", variant: "v3" } as const;
    if (key === "kling-3-0-video") return { provider: "kie", variant: "v3" } as const;
    return null;
}

function activeVideoChannelText(config: AiConfig) {
    const activeId = config.videoChannelId || config.activeChannelId;
    const channels = config.channelMode === "local" ? config.channels : config.publicChannels;
