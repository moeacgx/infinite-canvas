...
export function CanvasCameraControl({ value, onChange, buttonClassName }: CanvasCameraControlProps) {
    ...
    const panelStyle = (() => {
        if (!buttonRect || !viewport.width || !viewport.height) return undefined;
        const common = {
            position: "fixed",
            zIndex: 1200,
            overflowY: "auto",
        ...
        const scale = 0.75;
        const visibleWidth = 900 * scale;
        const center = Math.min(Math.max(buttonRect.left + buttonRect.width / 2, visibleWidth / 2 + 8), viewport.width - visibleWidth / 2 - 8);
        const bottom = Math.min(Math.max(viewport.height - buttonRect.top + 8, 8), viewport.height - 248);
        return {
            ...common,
            width: 900,
            maxHeight: Math.max(300, (viewport.height - bottom - 8) / scale),
            left: center,
            bottom,
        ...
        <>
            ...
            {open && buttonRect && panelStyle
                ? createPortal(
                      ...
                      >
                          <div className="flex items-center justify-between border-b px-4 py-3 sm:px-6 sm:py-4" style={{ borderColor: theme.toolbar.border }}>
...
function SettingColumn({ theme, separator, label, tooltipTitle, tooltipDesc, tooltipUseCase, badge, visual, caption, onPrevious, onNext }: SettingColumnProps) {
    ...
    return (
        <div className="flex flex-col items-center px-5" style={{ borderLeft: separator ? "1px solid " + theme.node.stroke : undefined }}>
            ...
            <Tooltip title={tooltip} mouseEnterDelay={0.7} color={theme.node.panel} zIndex={1300} styles={{ container: { color: theme.node.text } }}>
            ...
            <span className="min-h-10 max-w-full break-words text-center text-sm leading-5" style={{ color: theme.node.muted }}>{caption}</span>
        </div>
    );
}
