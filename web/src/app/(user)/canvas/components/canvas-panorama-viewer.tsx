...
function PanoramaSurface({ src, alt, proxyGeneratedPanorama, viewerEntry }: PanoramaSurfaceProps) {
    ...
    const [panoramaSrc, setPanoramaSrc] = useState(proxyGeneratedPanorama ? "" : src);

    useEffect(() => {
        setStatus("loading");
        if (!proxyGeneratedPanorama) {
            setPanoramaSrc(src);
            return;
        }
        let active = true;
        let objectUrl = "";
        void downloadRemoteImage(src)
            .then((blob) => {
                if (!active) return;
    ...
    }, [proxyGeneratedPanorama, src]);

    useEffect(() => {
        const container = containerRef.current;
        if (!container || !panoramaSrc) return;
