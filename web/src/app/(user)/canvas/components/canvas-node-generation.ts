...
import { getCanvasNodeResource, getGenerationResourceNodes } from "../utils/canvas-resource-references";
...
function inputToElementReference(input: NodeGenerationInput | undefined): VideoElementReference | null {
    ...
    if (input?.audio)
        return { id: input.nodeId, kind: "audio", name: input.audio.name, type: input.audio.type, url: input.audio.url, storageKey: input.audio.storageKey, durationMs: input.audio.durationMs };
    return null;
}
...
export function buildNodeGenerationInputs(nodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[]): NodeGenerationInput[] {
    return getGenerationResourceNodes(nodeId, nodes, connections).flatMap((node): NodeGenerationInput[] => {
        ...
        const image = readReferenceImage(node, resource);
        if (image) return [{ nodeId: node.id, type: "image" as const, title: node.title, image }];
        const video = readReferenceVideo(node, resource);
        if (video) return [{ nodeId: node.id, type: "video" as const, title: node.title, video }];
        const audio = readReferenceAudio(node, resource);
        if (audio) return [{ nodeId: node.id, type: "audio" as const, title: node.title, audio }];
        const text = resource?.kind === "text" ? resource.text || "" : readNodeTextInput(node);
...
export async function hydrateNodeGenerationContext(context: NodeGenerationContext, signal?: AbortSignal) {
    const { imageToDataUrl } = await import("@/services/image-storage");
    return {
        ...context,
        referenceImages: await Promise.all(context.referenceImages.map(async (image) => ({ ...image, dataUrl: await imageToDataUrl(image, signal) }))),
        firstFrame: context.firstFrame ? { ...context.firstFrame, dataUrl: await imageToDataUrl(context.firstFrame, signal) } : null,
        lastFrame: context.lastFrame ? { ...context.lastFrame, dataUrl: await imageToDataUrl(context.lastFrame, signal) } : null,
    };
}
...
function readReferenceImage(node: CanvasNodeData, resource = getCanvasNodeResource(node)): ReferenceImage | null {
    if (resource?.kind !== "image" || !resource.url) return null;
    return {
        id: node.id,
...
function readReferenceVideo(node: CanvasNodeData, resource = getCanvasNodeResource(node)): ReferenceVideo | null {
    if (resource?.kind !== "video" || !resource.url) return null;
    return {
        id: node.id,
...
function readReferenceAudio(node: CanvasNodeData, resource = getCanvasNodeResource(node)): ReferenceAudio | null {
    if (resource?.kind !== "audio" || !resource.url) return null;
    return {
        id: node.id,
