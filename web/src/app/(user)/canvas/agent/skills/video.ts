...
- quality：默认读取当前画布 Agent videoQuality；只有用户明确本次覆盖时传入。
- seconds：必须来自用户明确选择、镜头规划或全局 videoSeconds，并通过当前模型校验。
- generateAudio：视频文件是否直接生成声音；默认遵循全局配置，最终能力以 Provider 真实返回为准。
...
- 先用 get_generation_config 读取当前视频模型与 videoSeconds。
...
- 不依据模型名称静态猜测能力；以 Provider 真实返回为准。
