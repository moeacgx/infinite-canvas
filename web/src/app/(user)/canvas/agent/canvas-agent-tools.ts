...
export function normalizeCanvasAgentAction(name: unknown, args: unknown, id = nanoid()): CanvasAgentAction {
    ...
    switch (actionName) {
        ...
        case "edit_image": {
            ...
            const sourceNodeIds = requiredStringArray(input.sourceNodeIds, "sourceNodeIds");
            const background = optionalEnum(input.background, ["transparent", "opaque"], "background");
            normalized = {
                prompt,
                sourceNodeIds,
                ...(optionalString(input.title) ? { title: optionalString(input.title) } : {}),
                ...(optionalString(input.size) ? { size: optionalString(input.size) } : {}),
                ...(optionalString(input.quality) ? { quality: optionalString(input.quality) } : {}),
                ...(boundedInteger(input.count, 1, 15) ? { count: boundedInteger(input.count, 1, 15) } : {}),
                ...(background ? { background } : {}),
            };
            if (actionName === "edit_image" && !sourceNodeIds.length) throw new Error("edit_image 缺少图片来源节点");
            break;
        }
        case "generate_video": {
            const prompt = requiredString(input.prompt, "prompt");
...
function requiredStringArray(value: unknown, key: string) {
    if (!Array.isArray(value)) throw new Error(key + " 必须显式提供字符串数组；独立生成请传 []");
    return stringArray(value);
}

function optionalEnum<T extends string>(value: unknown, values: readonly T[], key: string): T | undefined {
    if (value === undefined || value === null || value === "") return undefined;
    if (typeof value !== "string" || !values.includes(value as T)) throw new Error(key + " 不是允许的值");
    return value as T;
}
