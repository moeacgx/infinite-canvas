...
export default function ImagePage() {
    ...
    const usesBackendImageTasks = (_value: AiConfig) => false;
    const imageTaskConfig = () => effectiveConfigRef.current;

    const updateWorkbenchConfig: UpdateAiConfig = (key, value) => {
        if (config.channelMode === "local" && key === "apiMode") {
            const channelId = resolveImageChannelId(config, model, config.imageChannelId) || config.channels[0]?.id || "";
            const apiMode = value === "responses" ? "responses" : "images";
            updateConfig(
                "channels",
                config.channels.map((channel) => (channel.id === channelId ? { ...channel, imageApiMode: apiMode } : channel)),
            );
        }
        if (config.channelMode === "local" && key === "imageModel") {
            const selectedModel = String(value || "");
            const channelId = resolveImageChannelId(config, selectedModel, decodeChannelModel(selectedModel)?.channelId, config.imageChannelId);
    ...
    const submitGenerationBatch = async (snapshot: RequestSnapshot) => {
        ...
        const tasks = taskIds.map(async (id, index) => {
            ...
            try {
                ...
                const durableImage: GeneratedImage = {
                    ...image,
                    dataUrl: stored.url,
    ...
    const persistImageHistory = async (_nextLogs: GenerationLog[], _nextCategories: GenerationCategory[]) => undefined;
    ...
    const buildRequestSnapshot = ({
        ...
        const requestModel = resolveCapabilityModel(baseConfig, "image", configOverride?.imageModel || configOverride?.model || model);
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                            <Input.TextArea
                                ...
                                onPressEnter={(event) => {
                                    if (event.nativeEvent.isComposing || event.nativeEvent.keyCode === 229 || event.shiftKey || event.ctrlKey || event.metaKey || !canGenerate || pendingCount) return;
                            ...
                            <label className="grid gap-1 text-xs text-stone-500 dark:text-stone-400">
                                ...
                                <div className="flex h-11 items-center rounded-xl border border-stone-200 bg-background px-2.5 dark:border-stone-800">
                                    <Segmented
                                        ...
                                        disabled={config.channelMode !== "local"}
...
function isAbortError(error: unknown, signal?: AbortSignal) {
    return Boolean(signal?.aborted) || (error instanceof DOMException && error.name === "AbortError") || (error instanceof Error && error.name === "AbortError");
}
