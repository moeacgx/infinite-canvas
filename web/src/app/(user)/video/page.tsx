...
import { getVideoMaxSeconds, modelKey, supportsVideoAudioGeneration, supportsVideoFrameReferences } from "@/lib/video-model-capabilities";
...
type VideoResponse = {
    ...
    request_body?: string;
    provider?: VideoGenerationTask["provider"];
    channelModel?: string;
    storageKey?: string;
    width?: number;
    height?: number;
...
class VideoRequestError extends Error {
    ...
    constructor(message: string, detail?: unknown) {
        ...
        this.detail = typeof detail === "string" ? detail : detail ? JSON.stringify(detail, null, 2) : undefined;
    }
}
...
const workbenchTaskSignals = new Map<string, AbortSignal>();

async function createVideoGenerationTask(config: AiConfig, prompt: string, input: ReferenceImage[] | VideoReferenceInput = [], onProgress?: (progress: number, task: VideoResponse) => void, options: WorkbenchTaskOptions = {}) {
    const normalized: Required<VideoReferenceInput> = Array.isArray(input)
        ? { references: input, firstFrame: null, lastFrame: null, videoReferences: [], audioReferences: [] }
        : {
    ...
    const task = await createBaseVideoGenerationTask(config, prompt, normalized, { signal: options.signal });
    const response: VideoResponse = {
        id: task.id,
        task_id: task.id,
        model: task.model,
        provider: task.provider,
        channelModel: task.channelModel,
        status: "queued",
        progress: 0,
    ...
    if (options.signal) workbenchTaskSignals.set(task.id, options.signal);
    onProgress?.(0, response);
    return {
        task: response,
        pollId: task.id,
        startedAt,
        requestBody: { prompt, model: task.model, source: options.source, clientTaskId: options.clientTaskId },
    };
}

async function pollVideoGenerationTaskStatus(config: AiConfig, task: VideoResponse) {
    const signal = workbenchTaskSignals.get(task.id);
    const provider = task.provider || (isSeedanceVideoConfig({ ...config, model: task.model || config.model }) ? "seedance" : "openai");
    try {
        const state = await pollBaseVideoGenerationTask(config, { id: task.id, provider, model: task.model || config.model || config.videoModel, channelModel: task.channelModel }, { signal });
        if (state.status === "pending") return { ...task, status: "processing" };
        if (state.status === "retrying") return { ...task, status: "processing", error: { message: state.error } };
        if (state.status === "failed") {
            workbenchTaskSignals.delete(task.id);
            return { ...task, status: "failed", error: { message: state.error } };
        }
        ...
        throw new VideoRequestError(error instanceof Error ? error.message : "视频任务查询失败", error);
    }
}

async function listVideoGenerationTasks(_config: AiConfig): Promise<VideoResponse[]> {
    return [];
}

async function deleteVideoGenerationTask(_config: AiConfig, task?: VideoResponse | null) {
    if (task?.id) workbenchTaskSignals.delete(task.id);
}
...
export default function VideoPage() {
    ...
    const filterCurrentModelAudioReferences = (existing: ReferenceAudio[], next: ReferenceAudio[]) => (isSeedanceVideoConfig({ ...videoConfig, model }) ? filterAudioReferencesByDuration(existing, next, message.warning) : next);
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        try {
            ...
            const nextAudioReferences = filterCurrentModelAudioReferences(
                audioReferences,
                await Promise.all(
    ...
    const buildRequestSnapshot = ({
        ...
        const seedance = isSeedanceVideoConfig({ ...configValue, model: modelValue });
        if (!text) {
        ...
        if (klingV26 && configValue.videoMode === "pro" && boolConfig(configValue.videoGenerateAudio, false)) {
    ...
    const submitGenerationSnapshot = async (snapshot: {
        ...
        try {
            ...
            const storedLogs = (await readStoredLogs()).filter((log) => !deletedLogIdsRef.current.has(log.id));
            setLogs(storedLogs);
    ...
    const runVideoTask = async (
        ...
        let createdTaskId = "";
        let createdTask: VideoResponse | null = null;
        const cleanupCreatedTask = async () => {
            controller.abort();
            taskControllersRef.current.delete(pendingLog.id);
            if (!createdTaskId) return;
            taskControllersRef.current.delete(createdTaskId);
            workbenchTaskSignals.delete(createdTaskId);
            if (createdTask) await deleteVideoGenerationTask(snapshot.config, createdTask).catch(() => undefined);
        };
        taskControllersRef.current.set(pendingLog.id, controller);
        try {
            const created = await createVideoGenerationTask(
                ...
                { clientTaskId: pendingLog.task?.id, source: "video-workbench", signal: controller.signal },
            );
            createdTaskId = created.task.id;
            createdTask = created.task;
            taskControllersRef.current.set(created.task.id, controller);
            if (deletedLogIdsRef.current.has(pendingLog.id)) {
            ...
            await cleanupCreatedTask();
            if (deletedLogIdsRef.current.has(pendingLog.id)) return null;
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        ...
        const insertAudio = () => {
            ...
            const next = filterCurrentModelAudioReferences(audioReferences, [{ id: nanoid(), name: payload.title, type: payload.mimeType || "audio/mpeg", url: payload.url, storageKey: payload.storageKey, durationMs: payload.durationMs }]);
    ...
    const deleteSelectedLogs = () => {
        ...
        deletedLogs.forEach((log) => {
            ...
            [log.id, taskId]
                ...
                .forEach((key) => {
                    taskControllersRef.current.get(key)?.abort();
                    taskControllersRef.current.delete(key);
                });
            if (taskId) workbenchTaskSignals.delete(taskId);
    ...
    const syncBackendVideoTasks = async (baseLogs?: GenerationLog[]) => {
        ...
        try {
            ...
            const mergedLogs = mergeBackendVideoTasks(currentLogs, recoverableTasks, config).filter((log) => !deletedLogIdsRef.current.has(log.id));
    ...
    const loadAccountVideoHistory = async (currentToken: string) => {
        try {
            ...
            const mergedLogs = (await mergeVideoLogs(remoteLogs, localLogs)).filter((log) => !deletedLogIdsRef.current.has(log.id));
    ...
    const persistVideoLog = async (log: GenerationLog) => {
        if (deletedLogIdsRef.current.has(log.id) || !token || !shouldSyncVideoLog(log)) return;
    ...
    const finalizeGenerationLog = async (log: GenerationLog) => {
        ...
        if (deletedLogIdsRef.current.has(log.id)) return;
        const nextLogs = (await readStoredLogs()).filter((item) => !deletedLogIdsRef.current.has(item.id));
        setLogs(nextLogs);
    ...
    const pollPendingLogOnce = async (log: GenerationLog, resumeConfig: AiConfig) => {
        ...
        try {
            ...
            pollingLogIdsRef.current.delete(log.id);
            if (terminal || deletedLogIdsRef.current.has(log.id)) {
                const taskId = videoLogTaskId(log);
                [log.id, taskId].filter((key): key is string => Boolean(key)).forEach((key) => taskControllersRef.current.delete(key));
                if (taskId) workbenchTaskSignals.delete(taskId);
            }
        }
    };
        ...
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            <main className={`${workbenchLayout === "side" ? "grid grid-cols-1 lg:grid-cols-[420px_minmax(0,1fr)]" : "relative flex flex-col"} min-h-0 flex-1 gap-3 overflow-y-auto p-3 lg:overflow-hidden`}>
                {workbenchLayout === "side" ? (
                    ...
                    <>
                        <ResultsPanel
                            className={`min-h-[360px] flex-1 ${workbenchLayout === "bottom" ? "h-full overflow-y-auto pb-56 sm:pb-52 lg:pb-52" : ""}`}
                            results={results}
                            logs={logs}
                            pendingCount={pendingCount}
                            now={now}
...
function WorkbenchPanel({
    ...
    const maxSeconds = getVideoMaxSeconds(config, model);
    ...
    if (layout === "bottom") {
        return (
            <div className="pointer-events-none fixed inset-x-0 bottom-5 z-40 flex justify-center px-5 sm:bottom-7 sm:px-10 lg:px-16">
                <div className="thin-scrollbar pointer-events-auto max-h-[calc(100dvh-2.5rem)] w-full max-w-5xl overflow-y-auto rounded-[24px] bg-white/65 p-4 shadow-[0_32px_100px_rgba(15,23,42,.22),0_10px_34px_rgba(15,23,42,.10)] ring-1 ring-white/50 backdrop-blur-2xl dark:bg-stone-950/60 dark:ring-white/10 dark:shadow-[0_34px_110px_rgba(0,0,0,.58)]">
                    <div className="flex flex-col gap-3">
                        <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                            ...
                            <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
                                ...
                                <Button type="primary" className="h-9 rounded-xl px-4 font-medium lg:!hidden" icon={<Sparkles className="size-4" />} disabled={!canGenerate || running} onClick={onGenerate}>
                        ...
                        <div className={`grid grid-cols-2 gap-2 sm:grid-cols-3 ${bottomSettingsGridClass} ${bottomSettingsCollapsed ? "hidden lg:grid" : "grid"}`}>
                            ...
                            {klingBottom ? (
                                ...
                                <>
                                    ...
                                    <QuickNumber label="秒数" value={normalizeVideoSeconds(config.videoSeconds)} min={1} max={maxSeconds} onChange={(value) => updateConfig("videoSeconds", value)} />
                            ...
                            <Button type="primary" className="hidden h-11 min-w-28 items-center justify-center gap-1.5 rounded-xl lg:flex" icon={<Sparkles className="size-4" />} disabled={!canGenerate || running} onClick={onGenerate}>
                                {pendingCount ? `${pendingCount} 生成中` : "开始创作"}
                            </Button>
                        </div>
    ...
    return (
        <div className="flex min-h-[420px] flex-col overflow-hidden rounded-lg border border-stone-200 bg-card shadow-sm dark:border-stone-800 lg:min-h-0">
            ...
            <div className="shrink-0 border-t border-stone-200 p-4 dark:border-stone-800">
                <Button type="primary" size="large" block icon={<Sparkles className="size-4" />} disabled={!canGenerate || running} onClick={onGenerate}>
...
function GenerationSettings({ config, model, updateConfig, openConfigDialog }: { config: AiConfig; model: string; updateConfig: UpdateAiConfig; openConfigDialog: (shouldPromptContinue?: boolean) => void }) {
    ...
    return (
        <div className="space-y-3">
            ...
            <VideoSettingsPanel config={config} model={model} onConfigChange={(key, value) => updateConfig(key, value)} theme={theme} showTitle={false} className="space-y-3" />
        </div>
    );
}
...
function ResultsPanel({
    ...
    onSaveAsset: (video: GeneratedVideo, prompt: string) => void;
        ...
        <section className={`thin-scrollbar rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto lg:p-5 ${className}`}>
            <div className="mb-4 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                    <HistoryIcon />
                    <h2 className="whitespace-nowrap text-lg font-semibold sm:text-xl">全部结果</h2>
                    <Tag className="m-0">{totalCount}</Tag>
                    {pendingCount ? <Tag className="m-0 hidden px-2 py-1 sm:inline-flex">{pendingCount} 个生成中</Tag> : null}
                </div>
                <div className="flex shrink-0 items-center gap-1 sm:gap-2">
                    <Button size="small" title="新建" aria-label="新建" icon={<Plus className="size-3.5" />} onClick={onCreateSession}>
                        <span className="hidden sm:inline">新建</span>
                    </Button>
                    <Button size="small" title={allSelected ? "取消全选" : "全选"} aria-label={allSelected ? "取消全选" : "全选"} icon={<CheckSquare className="size-3.5" />} disabled={!visibleLogs.length} onClick={toggleVisibleLogs}>
                        <span className="hidden sm:inline">{allSelected ? "取消" : "全选"}</span>
                    </Button>
                    <Button size="small" title="删除" aria-label="删除" danger icon={<Trash2 className="size-3.5" />} disabled={!selectedLogIds.length} onClick={onDeleteSelected}>
                ...
                <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
                    {visibleResults.map((result, index) =>
                        result.status === "success" && result.video ? (
                            <ResultVideoCard
                                ...
                                onSaveAsset={(video) => onSaveAsset(video, result.prompt)}
                            />
                    ...
                    {visibleLogs.map((log, index) => (
                        <HistoryLogCard
                            ...
                            onSaveAsset={(video) => onSaveAsset(video, log.prompt)}
                        />
                    ))}
                </div>
            ) : (
...
function VideoSourceTag({ video }: { video: GeneratedVideo }) {
    const cloud = isCloudVideo(video);
    return (
        <Tag className="m-0 text-[10px]" color={cloud ? "blue" : video.storageKey ? "default" : "gold"}>
            {cloud ? "云端存储" : video.storageKey ? "本地缓存" : "AI 临时URL"}
        </Tag>
    );
}
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    ...
    const normalizedConfig = normalizeLogConfig(log);
    const config = { ...normalizedConfig, videoElementList: await resolveStoredKlingElementList(normalizedConfig.videoElementList) };
    return {
        id: log.id || nanoid(),
...
function resolveVideoChannelId(config: AiConfig, model: string, ...preferredIds: Array<string | undefined>) {
    ...
    const encodedChannelId = decodeChannelModel(model)?.channelId;
    const supportsModel = (channel: (typeof channels)[number]) => channel.models.some((item) => modelOptionName(item) === modelName);
    for (const id of [encodedChannelId, ...preferredIds]) {
        const channelId = (id || "").trim();
        if (channelId && channels.some((channel) => channel.id === channelId && supportsModel(channel))) return channelId;
    }
    return channels.find(supportsModel)?.id || "";
}
...
function isAPIMartKlingModelConfig(config: AiConfig, model: string, key: string) {
    ...
    return channelText.includes("apimart") || config.channelMode === "newapi" || !channelText;
}

function isKIEKlingModelConfig(config: AiConfig, model: string, key: string) {
    if (modelKey(model) !== key) return false;
    const channelText = videoChannelText(config, model);
    return channelText.includes("kie") || (config.channelMode === "newapi" && key.startsWith("kling-3-0"));
}
...
function videoChannelText(config: AiConfig, model: string) {
    ...
    const modelName = modelOptionName(model);
    const supportsModel = (item: (typeof channels)[number]) => (item.models || []).some((value) => modelOptionName(value) === modelName);
    const channel = channels.find((item) => (item.id || "") === channelId && supportsModel(item)) || channels.find(supportsModel) || channels.find((item) => (item.id || "") === channelId);
...
async function resolveStoredKlingElementList(value: VideoElementItem[] | undefined): Promise<VideoElementItem[]> {
    return Promise.all(
        normalizeKlingElementList(value).map(async (item) => ({
            ...item,
            references: await Promise.all(
                item.references.map(async (reference) => {
                    if (!reference.storageKey) return reference;
                    if (reference.kind === "image") {
                        return { ...reference, dataUrl: await safeResolveImageUrl(reference.storageKey, reference.dataUrl || reference.url || "") };
                    }
                    return { ...reference, url: await safeResolveMediaUrl(reference.storageKey, reference.url || "") };
                }),
            ),
        })),
    );
