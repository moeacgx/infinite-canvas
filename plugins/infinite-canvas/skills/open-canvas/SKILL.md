...
```text
https://canvas.maolaoapi.com/canvas?mode=new&agentUrl=<Local URL>&agentToken=<Connect token>
```

## 本地版

1. 在 Infinite Canvas 项目中启动前端，并使用 Next.js 输出的 `Local` 地址：
...
```text
<Next.js Local 地址>/canvas?mode=new&agentUrl=<Local URL>&agentToken=<Connect token>
```

## MCP 与连接地址

插件在新的 Codex 任务中加载时会自动从本仓库最新 GitHub Release 启动 `canvas-agent mcp`。这个 MCP 进程负责提供画布工具，不提供网页连接服务；
